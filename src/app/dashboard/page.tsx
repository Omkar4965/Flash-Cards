...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFlashcards = async () => {
      setLoading(true);
      try {
        const res = await axios.get<Flashcard[]>(`http://localhost:3000/api/flashcards?user_id=${userId}`);
  ...
  const createHandler = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setLoading2(true);
    try {
      const res = await axios.post<{ id: string }>(`http://localhost:3000/api/flashcards?user_id=${userId}`, {
        flashcardtopics,
      });
