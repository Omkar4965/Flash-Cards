...
const FlashcardTopicPage: React.FC = () => {
  ...
  const submitHandler = async (que: string, ans: string) => {
    try {
      await axios.post('http://localhost:3000/api/queAns', {
        user_id: userId,
        flashcards_id,
        question: que,
        answer: ans,
  ...
  const deleteHandler = async (que: string, ans: string) => {
    try {
      await axios.delete('http://localhost:3000/api/queAns', {
        params: {
          user_id: userId,
          flashcards_id,
          question: que,
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [res, res2] = await Promise.all([
          axios.get(`http://localhost:3000/api/queAns?user_id=${userId}&flashcards_id=${flashcards_id}`),
          axios.get(`http://localhost:3000/api/flashcards?user_id=${userId}`),
        ]);
